...
//-*-*-*-* Toggle search view in mobile devices *-*-*-*-//
...
searchField.addEventListener("input", function () {
	// to avoid making immediate search requests with every keystroke
...
export const updateWeather = function (coords) {
	...
	fetchData(url.currentWeather(coords), function (currentWeather) {
		const {
			weather,
			dt: dateUnix,
		...
		fetchData(url.reverseGeo(coords), function ([{ name, country }]) {
		...
		fetchData(url.airPollution(coords), function (airPollution) {
			const [
				{
					main: { aqi },
		...
		fetchData(url.forecast(coords), function (forecastData) {
			const {
				list: forecastList,
				city: { timezone },
			} = forecastData;

			hourlySection.innerHTML = /* HTML */ `
			...
			for (const [index, data] of forecastList.entries()) {
				...
				windLi.innerHTML = /* HTML */ `
					<div class="card card-sm slider-card">
						...
						/>
						<p class="body-3">${parseInt(windSpeed)} km/h</p>
					</div>
				`;
